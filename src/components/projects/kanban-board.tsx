              ...
              <Droppable key={status} droppableId={status}>
                {(provided, snapshot) => (
                  <div
                    ...
                    className={`bg-muted/40 rounded-md p-3 transition-colors ${snapshot.isDraggingOver ? 'bg-primary/10' : ''}`}
                  >
                    ...
                    <ScrollArea className="h-[calc(100vh-300px)]">
                      <div className="space-y-2 pr-2">
                        {statusTasks.length === 0 ? (
                          <div className="text-center py-8 text-muted-foreground text-sm italic">No tasks</div>
                        ) : (
                            ...
                            <Draggable key={task.id} draggableId={task.id} index={index}>
                              {(providedDraggable, snapshotDraggable) => (
                                <div
                                  ...
                                  className={`bg-card p-3 rounded-md border shadow-sm transition-transform ${snapshotDraggable.isDragging ? 'rotate-1 shadow-lg' : ''}`}
                                >
